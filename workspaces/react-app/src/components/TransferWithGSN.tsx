...
export class TransferWithGSN extends Component {
  ...
  async TransferToken() {
    ...
    const res = await this.ctf!.TransferToken()
  ...
  async ApproveTokenOne() {
    this.setState({ status: 'sending' })
    // const res = await this.ctf!.capture()
    const res = await this.ctf!.ApproveTokenOne()
    this.setState({ status: 'txhash= waiting for mining' })
    const res2 = await res.wait()
    console.log('mined!')
    this.setState({ total: null, step: null, status: 'Mined in block: ' + res2.blockNumber })
  }
  async ApproveTokenTwo() {
    this.setState({ status: 'sending' })
    // const res = await this.ctf!.capture()
    const res = await this.ctf!.ApproveTokenTwo()
    this.setState({ status: 'txhash= waiting for mining' })
    const res2 = await res.wait()
    console.log('mined!')
    this.setState({ total: null, step: null, status: 'Mined in block: ' + res2.blockNumber })
  ...
  render () {
    ...
    // @ts-ignore
    return <>
      <h1>Transfer Token - Without Paying for Gas (Mumbai Network)</h1>
      Click the button to transfer token with your account, using GSN 3.0.0 (beta)
      <br/>
      ...
      <br/>
      <br/>
      <ActionButton title="Approve Token One (QBRIDGE) - Require Gas"
                    enabled={!this.state.account}
                    action={() => this.ApproveTokenOne()}
                    onError={(e?: Error) => {
                      console.log('==ex2', e)
                      this.setState({ error: e ? e.message : null })
                    }}/>
      <br/>
      <ActionButton title="Approve Token Two (QOIN) - Require Gas"
                    enabled={!this.state.account}
                    action={() => this.ApproveTokenTwo()}
                    onError={(e?: Error) => {
                      console.log('==ex2', e)
                      this.setState({ error: e ? e.message : null })
                    }}/>
      <br/>
      <ActionButton title="Transfer Multiple Token To Multiple Recipient"
                    enabled={!this.state.account}
                    action={() => this.TransferToken()}
                    onError={(e?: Error) => {
                      console.log('==ex2', e)
                      this.setState({ error: e ? e.message : null })
                    }}/>
